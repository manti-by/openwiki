...
**This is an OpenCode plugin** (`opencode-openwiki`), not a standalone app.
...
- **Wiki Agent** runs on `session.idle` (skips if wiki not initialized or transcript < 80 chars). Spawns a child session via `client.session.create` + `client.session.prompt`, expects pure JSON reply (`{skip:true}` or page content).
